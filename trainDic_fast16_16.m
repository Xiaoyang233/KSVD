...
stringDir = strcat(pathImag , '\' , dirImag(NoImag).name);
tempImag = imread(stringDir);
[blocks,idx] = my_im2col(tempImag,[blokSize,blokSize],sliding);
blocksExtr = randperm( length(idx),60000 );
blocksCluster = [blocksCluster, blocks( : ,blocksExtr)];
...
save trainDicPan16_16 Dksvd;


%% show results %%
I = displayDictionaryElementsAsImage(Dksvd, floor(sqrt(m)), floor(size(Dksvd,2)/floor(sqrt(m))),blokSize,blokSize,0);
